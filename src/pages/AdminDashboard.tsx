...
import { ShieldCheck, Users, Building2, CreditCard, Bell, LogOut, Plus, Send, MapPin, KeyRound, Loader2 } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    const googleIntegration = (integrations || []).find((i: any) => i.provider === "google_maps");
    setGoogleApiKey(googleIntegration?.api_key || "");
    setLoading(false);
  ...
  useEffect(() => {
    if (subDialogOpen) {
      setSubReference(generateReference());
    }
  }, [subDialogOpen]);
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          title="Transaction ID is auto-generated"
                        />
                      </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Manage the Maps API key landlords use to pin live locations on property listings.
                    </p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="google-api-key" className="flex items-center gap-2">
                    <KeyRound className="h-4 w-4" /> Google Maps API Key
                  </Label>
                  <Input
                    id="google-api-key"
                    type="password"
                  ...
                  <p className="text-xs text-muted-foreground">
                    Keep this key restricted to Maps URLs. Landlords can then drop live Google Maps links when adding properties, making them easy for tenants to find.
                  </p>
                </div>
                <Button
                  className="w-full sm:w-auto"
                  disabled={integrationLoading || savingIntegration}
                  onClick={async () => {
                    ...
                    const { error } = await supabase
                      ...
                      .upsert({ provider: "google_maps", api_key: googleApiKey.trim() });
                ...
                >
                  {savingIntegration && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
                  Save Integration
                </Button>
                <div className="rounded-md border p-3 bg-muted/50 text-sm leading-relaxed">
                  <p className="font-medium">Landlord experience</p>
                  <p>
                    Landlords can already paste a Google Maps link or capture their live location when adding a property.
                    Keeping this API key up to date ensures those links render correctly for tenants during search.
                  </p>
                </div>
              </CardContent>
            </Card>
